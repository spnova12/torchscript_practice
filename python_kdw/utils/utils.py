...
def read_one_from_yuvs(file_YUV_dir, w, h, start_frame, channel='y'):
    ...
    YUVs.seek(int(y_size * 1.5) * start_frame + channel_bytes)

    if channel == 'yuv':
        YUVs_buf = YUVs.read(int(y_size * 1.5))
        frame = np.frombuffer(YUVs_buf, dtype=np.uint8)
        frame_out = frame.copy()
    elif channel == 'y_u_v':
        YUVs_buf = YUVs.read(int(y_size * 1.5))
        yuv_byte = np.frombuffer(YUVs_buf, dtype=np.uint8)
        y = yuv_byte[0:y_size]
        ...
        YUVs_buf = YUVs.read(w * h)
        frame = np.reshape(np.frombuffer(YUVs_buf, dtype=np.uint8), [h, w])
        frame_out = frame.copy()
